...
class Window(QWidget):
    ...
    def Number_Of_Brackets(self, Function):
        counter_ri = 0
        counter_le = 0
        for Char in Function:
            if Char == ")":
    ...
    def Multiply_Divide_By_Nothing_Error(self, Function):
        ...
        for index in range(0, len(Function)):
            ...
            if Function[index] == "":
                continue
            elif Function[index][0] == "*" or Function[index][0] == "/" or Function[index][0] == "^" or Function[index][0] == ")" or Function[index][
                Last_Char_Index] == "*" or Function[index][Last_Char_Index] == "/" or Function[index][Last_Char_Index] == "^":
                M_D_Error[0] = True
    ...
    def Number_Followed_By_Variable_Error(self,Function):
        ...
        for index in range(0, len(Function)):
            for Char in Function[index]:
                if (Char in Identified_Numbers) or (Char == ")") or (Char == "x"):
                    Index_Of_Next_Char = Function[index].index(Char) + 1
                    if Index_Of_Next_Char < len(Function[index]):
                        if Char == "x" and Function[index][Index_Of_Next_Char]  in Identified_Numbers:
                            N_Vari_Error[0] = True
    ...
    def Repeated_Operation_Error(self,Function):
        ...
        for index in range(0, len(Function)):
            if ("**" in Function[index]) or ( "//" in Function[index]) or ( "^^" in Function[index]) or (Function[index] == "") or ( "()" in Function[index] ) or ( "xx" in Function[index] or ( "^)" in Function[index]) or ( "*)" in Function[index]) or ( "/)" in Function[index]) or ( "*/" in Function[index]) or ( "/*" in Function[index]) or ( "^/" in Function[index]) or ( "/^" in Function[index]) or ( "*^" in Function[index]) or ("^*" in Function[index])):
    ...
    def Checking_For_Errors(self,Function, Max_Value, Min_Value):
        Number_Of_Brac = self.Number_Of_Brackets(Function)
        Flag = 0
        ...
        if UnIdentified_Error[0]:
            ...
            if Mult_Div_Nothing_Error[0]:
                ...
                self.error("The Following Terms have a missing Operand or operation + or - followed by bracket -> ) " + str(Mult_Div_Nothing_Error))
                Flag = 1
            if Number_Of_Brac[0] != Number_Of_Brac[1]:
                self.error("There is a missing bracket")
                Flag = 1
            ...
            if Rep_Op_Error[0]:
                ...
                for Index in range(0, len(Rep_Op_Error)):
                    if Rep_Op_Error[Index] == "" and y == 0:
                        ...
                        y = 1
                self.error("The Following Terms have Repeated Operations or Repeated Variable or Empty Brackets or Operation * or / or ^ followed by -> )  " + str(Rep_Op_Error))
                Flag = 1
        return Flag
